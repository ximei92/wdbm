					...
					<div class="rows d-flex between">
						...
						<div class="board-search-box">
								...
								<c:if test="${type eq 'productNm'}">
									...
									<option value="thickness" >제품두께</option>
								</c:if>
								...
								<c:if test="${type eq 'thickness'}">
									<option value="productNm" >제품명</option>
									<option value="size">제품사이즈</option>
									<option value="thickness" selected>제품두께</option>
								</c:if>								
						...
						<div class="d-flex right wd40 gap10">
							...
							<button class="btn small bg_color1 ico add" onclick="location.href='goDetailRegist.do'">제품 등록</button>
						</div>
					</div>
					<!-- table -->
					<div class="bbs mt17">
						<table class="b-list">
							...
							<c:forEach items="${list}" var="data" varStatus="status">
								<tr>
									...
									<td><a onclick="location.href='goDetailRegist.do?idx=${list[status.index]['DETAIL_IDX']}'" class="tit">
									${list[status.index]['THICKNESS']}${list[status.index]['SIZE_NM']}-${list[status.index]['PRODUCT_CD']}</a></td>
						...
						<div class="pager txt-right">
						    <select name="contentnum" id="contentnum" onchange=detailPage()>
						    ...
						    <c:if test="${page.prev}">
                    			<a class="arr prev" href="javascript:detailPage(${page.getStartPage()-1});">이전</a>
                  			</c:if>
                  			<c:forEach begin="${page.getStartPage()}" end="${page.getEndPage()}" var="idx">
                    			<a href="javascript:detailPage(${idx});" <c:if test="${page.pagenum}== ${idx}">class="active"</c:if> >${idx}</a>
                  			</c:forEach>             			
							<c:if test="${page.next}">
                    			<a class="arr next"href="javascript:detailPage(${page.getEndPage()+1});">다음</a>
                  			</c:if>                  			
						</div>
					</div>
				</div>
